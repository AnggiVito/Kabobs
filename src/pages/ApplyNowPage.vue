    ...
    <q-page class="q-pa-md apply-now-page">
        <div class="apply-container q-mx-auto">
            ...
            <div class="intro-message q-mt-lg">
                <p>Terima kasih atas minat Anda untuk bergabung dengan tim kami! Untuk lebih mengenal Anda, mohon lengkapi semua bagian formulir ini. Formulir ini membutuhkan waktu sekitar 10-20 menit untuk diselesaikan.</p>
            ...
            <q-form @submit="onSubmit" class="q-gutter-md apply-form" ref="applicationForm">
                ...
                <div class="row q-col-gutter-md">
                    <div class="col-xs-12 col-sm-6">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.firstName" 
                            @update:model-value="val => applyNowStore.setFormField('firstName', val as string)" 
                            label="Nama Depan *" 
                            lazy-rules :rules="[val => !!val || 'Nama Depan wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.lastName" 
                            @update:model-value="val => applyNowStore.setFormField('lastName', val as string)" 
                            label="Nama Belakang *" 
                            lazy-rules :rules="[val => !!val || 'Nama Belakang wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.address" 
                            @update:model-value="val => applyNowStore.setFormField('address', val as string)" 
                            label="Alamat Lengkap *" 
                            type="textarea" rows="3" 
                            lazy-rules :rules="[val => !!val || 'Alamat Lengkap wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.phoneNumber" 
                            @update:model-value="val => applyNowStore.setFormField('phoneNumber', val as string)" 
                            label="Nomor Telepon *" 
                            type="tel" 
                            lazy-rules :rules="[val => !!val || 'Nomor Telepon wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.email" 
                            @update:model-value="val => applyNowStore.setFormField('email', val as string)" 
                            label="Email *" 
                            type="email" 
                            lazy-rules :rules="[val => !!val || 'Email wajib diisi', val => /.+@.+\..+/.test(val) || 'Format email tidak valid']" 
                        />
                    </div>
                    <div class="col-xs-12">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.linkedin" 
                            @update:model-value="val => applyNowStore.setFormField('linkedin', val as string | null)" 
                            label="URL Profil LinkedIn (Opsional)" 
                            type="url" :rules="[val => !val || /^(https?:\/\/)?(www\.)?linkedin\.com\/.*$/.test(val) || 'URL LinkedIn tidak valid']" 
                        />
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <q-select 
                            outlined :model-value="applyNowStore.getFormData.gender" 
                            @update:model-value="val => applyNowStore.setFormField('gender', val as string | null)" :options="genderOptions" 
                            label="Jenis Kelamin *" 
                            lazy-rules :rules="[val => !!val || 'Jenis Kelamin wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <q-select 
                            outlined :model-value="applyNowStore.getFormData.education" 
                            @update:model-value="val => applyNowStore.setFormField('education', val as string | null)" :options="educationOptions" 
                            label="Pendidikan Tertinggi *" 
                            lazy-rules :rules="[val => !!val || 'Pendidikan wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.fatherName" 
                            @update:model-value="val => applyNowStore.setFormField('fatherName', val as string)" 
                            label="Nama Lengkap Ayah Kandung *" 
                            lazy-rules :rules="[val => !!val || 'Nama Ayah Kandung wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.motherName" 
                            @update:model-value="val => applyNowStore.setFormField('motherName', val as string)" 
                            label="Nama Lengkap Ibu Kandung *" 
                            lazy-rules :rules="[val => !!val || 'Nama Ibu Kandung wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <q-select 
                            outlined :model-value="applyNowStore.getFormData.maritalStatus" 
                            @update:model-value="val => applyNowStore.setFormField('maritalStatus', val as string | null)" :options="maritalStatusOptions" 
                            label="Status Pernikahan *" 
                            lazy-rules :rules="[val => !!val || 'Status Pernikahan wajib diisi']" 
                        />
                    </div>
                </div>
                ...
                <div class="row q-col-gutter-md">
                    <div class="col-xs-12">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.previousJob" 
                            ...
                            lazy-rules :rules="[val => !!val || 'Informasi pekerjaan sebelumnya wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.whyKabobs" 
                            ...
                            type="textarea" 
                            rows="3" 
                            lazy-rules :rules="[val => !!val || 'Alasan bergabung dengan Kabobs wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.reasonForLeaving" 
                            ...
                            type="textarea" 
                            rows="3" 
                            lazy-rules :rules="[val => !!val || 'Alasan keluar dari pekerjaan sebelumnya wajib diisi']" 
                        />
                    </div>
                    <div class="col-xs-12">
                        <q-file 
                            outlined :model-value="applyNowStore.formData.cvFile" 
                            @update:model-value="newValue => applyNowStore.setFormField('cvFile', newValue)" 
                            label="Unggah CV (Hanya PDF) *" 
                            accept=".pdf" 
                            max-files="1" :rules="[val => !!val || 'CV wajib diunggah']">
                    ...
                    <div class="col-xs-12">
                        <q-input 
                            outlined :model-value="applyNowStore.getFormData.expectedSalary" 
                            @update:model-value="val => applyNowStore.setFormField('expectedSalary', val as number | null)" 
                            label="Ekspektasi Gaji (IDR) *" 
                            type="number" 
                            prefix="Rp" 
                            lazy-rules :rules="[val => !!val || 'Ekspektasi Gaji wajib diisi', val => val > 0 || 'Gaji harus angka positif']" 
                        />
                    </div>
                </div>
